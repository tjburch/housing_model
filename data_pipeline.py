...
class Dataloader:
    ...
    def __init__(self):
        ...
        self.ds_dict = {"sf": self.sf, "mf": self.mf, "condo": self.condo}

    def subset_to_interest(
        self, sf_maxprice=750000, mf_maxprice=1000000, hoa_maxprice=400
    ):
...
def create_data():
    ...
    avail_df["list_date"] = np.where(
        ...
        pd.Timestamp(pull_date)
        - pd.to_timedelta(avail_df["DAYS ON MARKET"], unit="days"),
        pd.to_datetime(avail_df["SOLD DATE"])
        - pd.to_timedelta(avail_df["DAYS ON MARKET"], unit="days"),
    )
    ...
    condo_df = avail_df_skim.query(
        "`PROPERTY TYPE` == 'Townhouse' | `PROPERTY TYPE` == 'Condo/Co-op'"
    )

    for df, name in zip([sf_df, mf_df, condo_df], ["sf", "mf", "condo"]):
